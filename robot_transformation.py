...
# # Created by Ke GUO in 07/03/2023
...
# define a function to move the world frame of the trajectory to the pose 0 frame
def shift0(traj):
    """
    define a function to move the world frame of the trajectory to the pose 0 frame
    Args:
    ...
    Returns:
        translate and rotate the traj
    """
    traj_ses = pos_quats2SE(np.array(traj))
...
# define a function to get the translation from one pose
def pose2trans(pose_data):
    """
    define a function to get the translation from one pose
    Args:
        pose_data (_type_): _description_
...
# define a function to resalce the input pose based on the pose_gt
def rescale(poses_gt, poses):
    """
    define a function to resalce the input pose based on the pose_gt, similar to rescale
    Args:
        poses_gt (_type_): N x 7 poselist in quaternion format
        poses (_type_): N x 7 poselist in quaternion format
    Returns:
        _type_: _description_
    """
...
# define a function to move the orgin of the gt_traj and est_traj to the start frame
...
# define a function to move the orgin of the gt_traj and est_traj to the start frame and get the scale
def transform_trajs(gt_traj, est_traj, cal_scale):
    """define a function to move the orgin of the gt_traj and est_traj to the start frame and get the scale

    Args:
        gt_traj (_type_): _description_
        est_traj (_type_): _description_
        cal_scale (_type_): _description_
...
# define a function to transfer the gt and estimated trajectory to the SE form
def quats2SEs(gt_traj, est_traj):
    ...
    Returns:
        _type_: _description_
    """
    gt_SEs = pos_quats2SE(gt_traj)
    est_SEs = pos_quats2SE(est_traj)
...
def get_relative_pose(pose0: np.ndarray, pose1: np.ndarray):
    ...
    relative_pose_mat = inverse_transformation(pose0_mat) @ pose1_mat
